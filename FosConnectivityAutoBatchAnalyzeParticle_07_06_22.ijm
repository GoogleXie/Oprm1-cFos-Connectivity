...
Dialog.addString("ROI Name: ", "Cortex");
Dialog.show();
...
Dialog.addString("Sample Name of interest: ", "1,2,4,6,7,8,12,13,14,16,17,19,30,31,32,33,40,41,42,43");
Dialog.addString("Delimiter Type: ", " ")
Dialog.addString("Group Name type: ", "Slice")
Dialog.addString("Group Name of interest: ", "1,2,3");
Dialog.show();
...
Dialog.addString("Image Known Width in pixel: ", "960,960,960");
Dialog.addString("Image Known Width: ", "3623,3623,3623");
...
Dialog.addString("Particle Size Range: ", "2-500");
...
function SetROI(ROI_Directory_To_Save, imageName, ImageNameArray, Delimiter, SliceGroup){
	...
	if(ImageNameArray[1] == Sample_Name[0]){
		*/
		waitForUser("Draw ROI, then click t to add. Add as many as you want."); // ask user to draw the roi
    	roiManager("deselect");
		roiManager("Combine");
		roiManager("delete");
		roiManager("Add");
